...
export const signin = async(req,res)=>{
    try{
        ...
        if(!isMatch)
            returnres.status(400).json({msg:"Invalid credentials"});
        ...
        res.json({msg:"Sign in sucessful",token})
    }catch(err){
        res.status(500).json({msg:"Sign in failed", error:err.msg})
    }
...
export const verifyEmail = async (req, res) => {
    try {
        ...
        if (!verificationToken)
            return res.status(400).json({ msg: "Invaild or expired token" })
        await User.findByIdAndUpdate(verificationToken.userId, { verified: true });
